 ...
 */
function scoreQuery(query: string): Record<IntentType, number> {
  ...
  for (const [intent, pattern] of Object.entries(INTENT_KEYWORDS) as [IntentType, RegExp][]) {
...
export function classifyIntent(query: string): IntentResult {
  ...
  const [topIntent, topScore] = entries.reduce(
    (best, cur) => (cur[1] > best[1] ? cur : best),
    ['troubleshooting' as IntentType, 0] as [IntentType, number]
  );
