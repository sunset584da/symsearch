 ...
 */
function generateFollowUpQueries(query: string): [string, string] {
  const q = query.trim();
 ...
 */
function avgRelevance(results: SearchResult[]): number {
 ...
 */
export async function maybeChain(
  query: string,
  results: SearchResult[],
  runSearch: (q: string) => Promise<SearchResult[]>
): Promise<SearchResult[]> {
  const needsChaining =
    results.length < 3 || avgRelevance(results) < 0.4;
